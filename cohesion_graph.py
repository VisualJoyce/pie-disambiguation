...
def cohesion_graph(data, use_test_data, embeddings_dir, graph_size, graph_pos, graph_pie, graph_set, graph_lemma,
    ...
    embeddings['UNK'] = numpy.mean(numpy.array(list(embeddings.values())), axis=0)
    print('Done! Took {0:.2f} seconds'.format(time.time() - time_0))
    ...
    for PIE in data:
        if (use_test_data and PIE.split == 'test') or (not use_test_data and PIE.split == 'dev'):
            ...
            if graph_definitions:
                ...
                graph_literal = construct_graph(PIE, pos_tagger, graph_size, graph_pos, graph_pie, graph_set,
                                                graph_lemma, graph_intra, graph_definitions)
                ...
                graph_figurative = construct_graph(PIE_figurative, pos_tagger, graph_size, graph_pos, graph_pie,
                                                   graph_set, graph_lemma, graph_intra, graph_definitions)
                ...
                graph = construct_graph(PIE, pos_tagger, graph_size, graph_pos, graph_pie, graph_set, graph_lemma,
                                        graph_intra, graph_definitions)
    ...
    print('Empty graphs: {0}. No-PIE-graphs: {1}. No-context-graphs: {2}.'.format(num_empty_graphs, num_no_pie_graphs,
                                                                                  num_no_context_graphs))
    if graph_definitions:
...
def construct_graph(PIE, pos_tagger, graph_size, graph_pos, graph_pie, graph_set, graph_lemma, graph_intra,
                    graph_definitions):
    """
    ...
    if graph_size[-1] == 'w':
        ...
        PIE_context_tokens = [token for token in content_tokens if
                              token.i >= PIE_tokens[0] and token.i <= PIE_tokens[-1]]
            ...
            if token_1.i not in PIE_tokens:
                for token_2 in content_tokens:
                    if token_2.i in PIE_tokens:
                        if (not graph_lemma and token_1.lower_ == token_2.lower_) or (
                                graph_lemma and token_1.lemma_ == token_2.lemma_):
...
def predict_label_contrast(graph_literal, graph_figurative, graph_type):
    ...
    if not [edge for edge in graph_literal if edge.in_PIE] and not [edge for edge in graph_figurative if edge.in_PIE]:
        ...
        if predict_label(graph_figurative, graph_type) == 'i':
            ...
            return 'i'
    # If no context words in the graph, label idiomatic
    if not [edge for edge in graph_literal if not edge.in_PIE] or not [edge for edge in graph_figurative if
                                                                       not edge.in_PIE]:
        num_no_context_graphs += 1
        return 'i'
