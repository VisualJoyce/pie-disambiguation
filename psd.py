...
if __name__ == '__main__':
    ...
    pie_corpus = read_corpus.read_PIE_Corpus(config.PIE_DIR, config.BNC_DIR, config.CONTEXT_SIZE, config.WORK_DIR,
                                             config.NO_CACHE)
    ...
    semeval = read_corpus.read_SemEval(config.SEMEVAL_DIR, config.SEMEVAL_CONTEXTS, config.CONTEXT_SIZE,
                                       config.WORK_DIR, config.NO_CACHE)
    print(semeval[180].__dict__)
    print('Read {0} PIEs from SemEval-2013 Task 5b corpus.'.format(len(semeval)))

    # # Read in the IDIX corpus
    # idix = read_corpus.read_IDIX(config.IDIX_DIR, config.BNC_DIR, config.CONTEXT_SIZE, config.WORK_DIR, config.DATA_DIR,
    #                              config.NO_CACHE)
    # print(idix[1].__dict__)
    # print('Read {0} PIEs from IDIX corpus.'.format(len(idix)))

    # Read in the VNC-Tokens dataset
    vnc = read_corpus.read_VNC(config.VNC_DIR, config.BNC_DIR, config.CONTEXT_SIZE, config.WORK_DIR, config.DATA_DIR,
                               config.NO_CACHE)
    print(vnc[10].__dict__)
    print('Read {0} PIEs from VNC-Tokens dataset.'.format(len(vnc)))
    # Combine datasets
    # combined_data = vnc + idix + semeval + pie_corpus
    combined_data = vnc + semeval + pie_corpus
    ...
    dev_corpora = ['VNC-dev', 'VNC-skewed', 'SemEval-lexsample-train', 'SemEval-allwords-train',
                   'SemEval-lexsample-dev', 'SemEval-allwords-dev', 'IDIX-single', 'PIE-train', 'PIE-dev']
    ...
    if config.METHOD == 'mfs':
        ...
        combined_data = cohesion_graph.cohesion_graph(combined_data, config.TEST, config.EMB_DIR, config.GRAPH_SIZE,
                                                      config.GRAPH_POS, config.GRAPH_PIE, config.GRAPH_SET,
                                                      config.GRAPH_LEMMA, config.GRAPH_GLOVE, config.GRAPH_INTRA,
                                                      config.GRAPH_TYPE, config.GRAPH_DEFINITIONS)
    print('Classified {0} of {1} as idiomatic.'.format(len([x for x in combined_data if x.predicted_label == 'i']),
                                                       len([x for x in combined_data if x.predicted_label])))
    # Run evaluation?
    user_input = input("\nRun evaluation? (y/n): ")
    ...
    if user_input.lower() == 'y':
        pickle.dump(combined_data, open(os.path.join(config.WORK_DIR, 'predictions_{0}.pickle'.format(
            '{:%Y-%m-%d-%H-%M-%S}'.format(datetime.datetime.now()))), 'wb'))
